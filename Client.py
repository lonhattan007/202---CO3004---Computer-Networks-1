...
class Client:
	...
	def sendRtspRequest(self, requestCode):
		...
		if requestCode == self.SETUP and self.state == self.INIT:
			...
			request='SETUP {} RTSP/1.0\n' \
					'CSeq: {}\n' \
					'Transport: RTP/UDP; ' \
					'client_port= {}'.format(str(self.fileName), str(self.rtspSeq), str(self.rtpPort))
		...
		# Play request
			...
			request = 'PLAY {} RTSP/1.0\n' \
					  'CSeq: {}\n' \
					  'Session: {}'.format(str(self.fileName), str(self.rtspSeq), str(self.sessionId))
	...
	def openRtpPort(self):
		...
		self.rtspSocket.settimeout(500)
